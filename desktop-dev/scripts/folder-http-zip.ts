...
import { $AppMetaData } from "../src/sys/jmm/jmm.ts";
...
const bundleDir = flags._.slice().shift();
if (
  typeof bundleDir !== "string" ||
  fs.existsSync(bundleDir) === false ||
  fs.statSync(bundleDir).isDirectory() === false
) {
  throw new Error(`need input 'YOUR/FOLDER/FOR/BUNDLE'`);
}

const bundleName = (flags.name || path.basename(bundleDir)) + ".zip";
const bundleMime = mime.getType(bundleName) || "application/octet-stream";
const metadataName = "metadata.json";
const metadataMime = mime.getType(metadataName) || "application/octet-stream";
const defaultMetadata: $AppMetaData = {
  ...
  title: "Demo",
  subtitle: "plaoc demo",
  introduction: "This is WebApp bundle by plaoc",
...
const tryReadMetadata = () => {
  try {
    return {
      ...defaultMetadata,
      ...JSON.parse(
        fs.readFileSync(path.join(bundleDir, metadataName), "utf-8")
      ),
    } as $AppMetaData;
  } catch {
...
export const bundleToZip = async () => {
  ...
  for (const entry of WalkAny(bundleDir)) {
    if (entry.isFile) {
      ...
      zip.file(entry.relativepath, null, {
        dir: true,
        date: entry.stats.mtime,
      });
    }
...
http
  .createServer(async (req, res) => {
    if (req.url === "/" + bundleName) {
      res.setHeader("Content-Type", bundleMime);
      const zip = await bundleToZip();
      zip
        .generateNodeStream({ compression: "STORE" })
        .pipe(res as NodeJS.WritableStream);
      return;
    } else if (req.url === "/" + metadataName) {
      /// 动态生成 合成metadatah
      res.setHeader("Content-Type", metadataMime);
    ...
    try {
      const filepath = path.join(bundleDir, req.url || "/index.html");
  ...
  .listen(port, () => {
    for (const info of Object.values(os.networkInterfaces())
      ...
      console.log(`meta: \thttp://${info?.address}:${port}/${metadataName}`);
      console.log(`zip: \thttp://${info?.address}:${port}/${bundleName}`);
    }
  });
