...
import info.bagen.dwebbrowser.ui.entity.MediaInfo
import info.bagen.dwebbrowser.ui.entity.MediaType
import info.bagen.dwebbrowser.ui.entity.createContentValues
import info.bagen.dwebbrowser.util.createMediaInfo
import java.io.File
...
object MediaDBManager {
    ...
    fun saveMediaInfoList(saveMaps: HashMap<String, ArrayList<MediaInfo>>) {
        ...
        saveMaps.forEach {
            ...
            it.value.forEach { mediaInfo ->
                ...
                if (ops.size > 0 && ops.size % 100 == 0) {
                    App.appContext.contentResolver.applyBatch(
        ...
        if (ops.size > 0) {
            App.appContext.contentResolver.applyBatch(AppContract.AUTHORITY, ops)
            ops.clear()
        }
    }
    ...
    fun insertMediaInfoByPath(path: String) {
        ...
        mediaInfo?.let { mi ->
            val uri = App.appContext.contentResolver.insert(
    ...
    fun updateMediaInfoByPath(path: String) {
        ...
        mediaInfo?.let { mi ->
            ...
            val count = App.appContext.contentResolver.update(
    ...
    fun deleteMediaInfoByPath(path: String) {
        val selection = "${AppContract.Medias.COLUMN_PATH} = ?"
        val selectionArgs = arrayOf("$path")
        val count = App.appContext.contentResolver.delete(
