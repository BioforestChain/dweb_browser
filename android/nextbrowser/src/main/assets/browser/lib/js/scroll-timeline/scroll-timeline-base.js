...
export function calculateMaxScrollOffset(source, axis) {
  ...
  if (axis === "vertical") return source.scrollHeight - source.clientHeight;
...
function resolvePx(cssValue, resolvedLength) {
  if (cssValue instanceof CSSUnitValue) {
    ...
    else if (cssValue.unit == "px") return cssValue.value;
    else throw TypeError("Unhandled unit type " + cssValue.unit);
...
function updateSource(timeline, source) {
  ...
  if (oldSource && oldScrollListener) {
    scrollEventSource(oldSource).removeEventListener(
      "scroll",
      oldScrollListener
    );
  }
...
export class ScrollTimeline {
  ...
  set axis(axis) {
    if (["block", "inline", "horizontal", "vertical"].indexOf(axis) === -1) {
...
export function calculateRange(phase, container, target, axis, optionsInset) {
  ...
  if (
    ...
    viewPos = left;
    if (rtl) viewPos += container.scrollWidth - container.clientWidth;
...
function parseInset(value, containerSize) {
  ...
  parts.forEach((part) => {
    ...
    else if (part.endsWith("px")) insetParts.push(parseFloat(part));
    else if (part === "auto") insetParts.push(0);
  });
