...
var _IpcResponse = class extends IpcBody {
  ...
  static fromStream(req_id, statusCode, stream, headers = new IpcHeaders(), ipc) {
    ...
    const ipcResponse = new _IpcResponse(
      req_id,
      statusCode,
      ipc.support_message_pack ? [24 /* BINARY_STREAM_ID */, stream_id] : [20 /* BASE64_STREAM_ID */, stream_id],
      headers.toJSON(),
      ipc
    );
...
var createHttpDwebServer = async (microModule, options) => {
  ...
  const close = (0, import_once2.default)(() => unlistenHttpDwebServer(microModule, options));
  return { origin, token, start, close };
};
var handleHttpDwebServer = async (microModule, token) => {
...
var listenHttpDwebServer = (microModule, options) => {
...
var unlistenHttpDwebServer = async (microModule, options) => {
  return microModule.fetch(
...
var assets_default = `<html lang="en">\r
<head>\r
    <meta charset="UTF-8">\r
    <meta http-equiv="X-UA-Compatible" content="IE=edge">\r
    <meta name="viewport" content="width=device-width, initial-scale=1.0">\r
    <title>statusbar</title>\r
...
<body>\r
    ...
    <script type="text/javascript">\r
        // const channel = new MessageChannel()\r
        fetch("./port")\r
\r
        // .then(res => console.log('\u6210\u529F'))\r
        // .catch(err => console.log('\u5931\u8D25'))\r
        \r
    <\/script>\r
</body>\r
</html>`;

// src/sys/statusbar/statusbar.worker.mts
var main = async () => {
