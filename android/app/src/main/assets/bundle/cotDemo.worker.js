...
var _IpcBodySender = class extends IpcBody {
   ...
   */
  useByIpc(ipc2) {
    if (this.usedIpcMap.has(ipc2)) {
      return true;
    }
    if (this.isStream && !this._isStreamOpened) {
      this.usedIpcMap.set(ipc2, 0);
      this.closeSignal.listen(() => {
        this.unuseByIpc(ipc2);
      });
      return true;
    }
    return false;
   ...
   * 拉取数据
   */
  emitStreamPull(message, ipc2) {
    const pulledSize = this.usedIpcMap.get(ipc2) + message.desiredSize;
    this.usedIpcMap.set(ipc2, pulledSize);
   ...
   */
  unuseByIpc(ipc2) {
    if (this.usedIpcMap.delete(ipc2) != null) {
...
var _IpcResponse = class extends IpcMessage {
  ...
  static async fromResponse(req_id, response, ipc2) {
    let ipcBody;
    if (response.body) {
...
jsProcess.onConnect((ipc2) => {
  ipc2.onEvent((event) => {
    const observers = ipcObserversMap.get(ipc2.remote.mmid);
...
var main = async () => {
  ...
  (await wwwServer.listen()).onRequest(async (request, ipc2) => {
    ...
    console.log(import_chalk.default.red("\u8FD9\u91CC\u7684 ipc.postMessage \u6CA1\u6709\u529E\u6CD5\u89E6\u53D1\u8FD4\u56DE"));
    ipc2.postMessage(
      new IpcResponse2(
        request.req_id,
  ...
  const url = encodeURIComponent(
    wwwServer.startResult.urlInfo.buildInternalUrl((url2) => {
      url2.pathname = "/index.html";
    }).href
  );
  console.log("\u5F00\u59CB \u6253\u5F00 cotdemo \u7A0B\u5E8F url: ", url);
  {
    const interUrl = wwwServer.startResult.urlInfo.buildInternalUrl((url2) => {
      url2.pathname = "/index.html";
    }).href;
