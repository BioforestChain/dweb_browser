...
async function createApiServerOnRequest(internal_origin) {
  return async (ipcRequest, ipc2) => {
    apiServerOnRequest(ipcRequest, ipc2, internal_origin);
  };
}
async function apiServerOnRequest(ipcRequest, ipc2, internal_origin) {
  ...
  switch (pathname) {
    case (pathname.startsWith("/status-bar.sys.dweb") ? pathname : symbolETO):
      apiServerOnRequestStatusbar(ipcRequest, ipc2, pathname, internal_origin);
      break;
  }
}
