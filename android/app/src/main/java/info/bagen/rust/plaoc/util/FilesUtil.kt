...
import info.bagen.rust.plaoc.ui.entity.loadThumbnail
import info.bagen.rust.plaoc.util.FilesUtil.getFileType
import java.io.File
    ...
    try {
      ...
      if (fileNames != null) {
        if (fileNames.isNotEmpty()) { // 目录
          ...
          var inputStream = App.appContext.assets.open(oldPath)
