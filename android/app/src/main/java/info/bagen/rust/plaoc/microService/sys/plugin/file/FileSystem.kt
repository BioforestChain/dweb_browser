...
//      Log.i("checkFilter ${file.name}: ", "文件类型=$rType，匹配规则=$rName")
    ...
    fun ls(path: String, filter: Array<LsFilter>, recursive: Boolean = false) {
        ...
        if (check != "ok") {
//            return createBytesFactory(ExportNative.FileSystemLs, check) // TODO
        }
...
//        createBytesFactory(ExportNative.FileSystemLs, JsonUtil.toJson(fileList)) // TODO return
    }
...
//    Log.i("FileSystemList, $rootPath", it.absolutePath)
    ...
    fun mkdir(path: String, recursive: Boolean = false) {
        ...
        // TODO return  createBytesFactory(ExportNative.FileSystemMkdir, bool.toString())
    }
...
//    LogUtils.d("write  file->${file.absolutePath}，content->$content，append->$append，autoCreate->$autoCreate")
        try {
            ...
            // TODO return    return createBytesFactory(ExportNative.FileSystemWrite, e.message.toString())
        }
        // TODO return  createBytesFactory(ExportNative.FileSystemWrite, true.toString())
    }
    ...
    fun read(path: String) {
        ...
        try {
            ...
            e.printStackTrace()
            // TODO return    createBytesFactory(ExportNative.FileSystemReadBuffer, e.message.toString())
        } catch (e: IOException) {
            e.printStackTrace()
            // TODO return    createBytesFactory(ExportNative.FileSystemReadBuffer, e.message.toString())
        }
        LogUtils.d("read buffer.toString -> $buffer")
        // TODO return  createBytesFactory(ExportNative.FileSystemRead, buffer.toString())
    }
    ...
    fun readBuffer(path: String) {
        ...
        // TODO return   createBytesFactory(ExportNative.FileSystemReadBuffer, buffer.toString())
    }
    ...
    fun rename(path: String, newFilePath: String) {
        ...
        if (!ordFile.exists()) {
            // TODO return    createBytesFactory(ExportNative.FileSystemRename, "重命名文件不存在")
        }
        if (newFile.exists()) {
            // TODO return   createBytesFactory(ExportNative.FileSystemRename, "重命名文件冲突，文件已存在")
        }
        try {
            val bool = ordFile.renameTo(newFile)
            // TODO return    createBytesFactory(ExportNative.FileSystemRename, bool.toString())
        } catch (e: IOException) {
            e.printStackTrace()
            // TODO return    createBytesFactory(ExportNative.FileSystemRename, e.message.toString())
        }
    }
    ...
    fun rm(path: String, deepDelete: Boolean = true) {
        ...
        // TODO return   createBytesFactory(ExportNative.FileSystemRm, bool.toString())
    }
    ...
    fun stat(path: String) {
        ...
        // TODO return   createBytesFactory(ExportNative.FileSystemStat, data.toString())
    }
}
