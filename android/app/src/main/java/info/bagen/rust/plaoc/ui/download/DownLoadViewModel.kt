...
import info.bagen.rust.plaoc.network.base.IApiResult
...
sealed class DownLoadIntent {
  class LoadDownLoadStateAndDownLoad(val path: String, val appInfo: AppInfo? = null) :
    DownLoadIntent()
...
class DownLoadViewModel(
    ...
    viewModelScope.launch(Dispatchers.IO) {
      ...
      when (action) {
        is DownLoadIntent.LoadDownLoadStateAndDownLoad -> {
          uiState.value.downloadAppInfo = action.appInfo
  ...
  private suspend fun downLoadAndSave(url: String, saveFile: String) {
    repository.downLoadAndSave(url, saveFile, isStop = {// 用来控制是否停止下载
      ...
      it.fold(onSuccess = { file ->
        if (uiState.value.downloadAppInfo == null) {
  ...
  private suspend fun decompressFile() {
    // 解压文件
    val enableUnzip = ZipUtil.ergodicDecompress(
      filePath = mDownLoadProgress.downloadFile,
      outputDir = FilesUtil.getAppUnzipPath(),
      mmid = uiState.value.downloadAppInfo?.bfsAppId
    )
    if (enableUnzip) {
