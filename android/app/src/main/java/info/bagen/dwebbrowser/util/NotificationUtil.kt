...
class NotificationUtil {
  ...
  private constructor() {
    mManager = App.appContext.getSystemService(NOTIFICATION_SERVICE) as NotificationManager
   ...
   * 普通通知
   */
  fun createNotificationForNormal() {
    // 适配8.0及以上 创建渠道
    ...
    // 构建配置
    mBuilder =
      NotificationCompat.Builder(App.appContext, mNormalChannelId).setContentTitle("普通通知") // 标题
  ...
  fun createNotificationForHigh() {
    ...
    mBuilder = NotificationCompat.Builder(App.appContext, mHighChannelId).setContentTitle("重要通知")
  ...
  fun createNotificationForBigText() {
    ...
    mBuilder =
      NotificationCompat.Builder(App.appContext, mBigTextChannelId).setContentTitle("大文本通知")
