...
import info.bagen.dwebbrowser.ui.download.DownLoadIntent
import info.bagen.dwebbrowser.ui.download.DownLoadViewModel
...
class AppViewModel(private val repository: AppRepository = AppRepository()) : ViewModel() {
    ...
    viewModelScope.launch(ioAsyncExceptionHandler) {
      ...
      when (action) {
        ...
        is AppViewIntent.ShareAppMenu -> {
          ...
          App.appContext.startActivity(shareIntent)
        }
  ...
  private suspend fun newAppDownloadAndInstall(path: String) {
    ...
    if (!found) {
      PlaocUtil.showShortToastMessage("开始下载")
      ...
      PlaocUtil.showShortToastMessage("正在下载中，不需要重新下载...")
    }
  }
