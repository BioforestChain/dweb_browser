    ...
    public override bool FinishedLaunching(UIApplication application, NSDictionary launchOptions)
    {
        ...
        Func<PureRequest, Task<PureResponse>> httpHanlder = async (request) =>
        {
            if (request.ParsedUrl is not null and var parsedUrl && parsedUrl.FullHost is "localhost:20222")
            {
                ...
                {
                    request.ParsedUrl = new URL(parsedUrl.Path.Substring(HttpNMM.X_DWEB_HREF.Length));
                }
            }
            if (request.ParsedUrl?.Path is "/index.html")
            {
                var response = new PureResponse(HttpStatusCode.OK, new(new[] { ("Content-Type", "text/html, charset=utf-8") }), new PureUtf8StringBody($$"""
                    <h1>你好!!</h1>
                    <h1>你好!!</h1>
                    <h1>你好!!</h1>
                    <h1>你好!!</h1>
