...
public partial class DeskController
{
    ...
    public URL GetDesktopUrl() =>
        new(DesktopServer.StartResult.urlInfo.BuildInternalUrl().Path("/desktop.html"));
        
    public URL GetDesktopApiUrl() =>
        GetDesktopUrl().SearchParamsSet("api-base", DesktopServer.StartResult.urlInfo.BuildPublicUrl().ToString());
}
