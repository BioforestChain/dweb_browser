...
namespace DwebBrowser.HelperTests
{
	public class UriExtensionsTests
	{
		...
		public void Path_Replace_ReturnSuccess(string url, string path)
		{
			...
			Debug.WriteLine($"{uri.AbsoluteUri.Substring(0, uri.AbsoluteUri.Length-1)}{path.Replace("%2f", "/")}");
			Assert.Equal(new Uri("https://js.sys.dweb:80/%3cinternal%3e/bootstrap.js"),
				new Uri($"{uri.AbsoluteUri.Substring(0, uri.AbsoluteUri.Length - 1)}{path.Replace("%2f", "/")}"));
		}
	}
}