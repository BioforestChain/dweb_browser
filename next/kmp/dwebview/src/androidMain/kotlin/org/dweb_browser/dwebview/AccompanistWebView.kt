...
import org.dweb_browser.dwebview.LoadingState.Finished
import org.dweb_browser.dwebview.LoadingState.Loading
...
public fun AccompanistWebView(
  ...
  BoxWithConstraints(modifier) {
    ...
    // layout params here.
    val width = if (constraints.hasFixedWidth) LayoutParams.MATCH_PARENT
    else LayoutParams.WRAP_CONTENT
    val height = if (constraints.hasFixedHeight) LayoutParams.MATCH_PARENT
    else LayoutParams.WRAP_CONTENT
        ...
        when (content) {
          ...
          is WebContent.Data -> {
            wv.loadDataWithBaseURL(
              content.baseUrl, content.data, content.mimeType, content.encoding, content.historyUrl
            )
          }

          is WebContent.Post -> {
            wv.postUrl(
              content.url, content.postData
            )
          }
    ...
    navigationEvents.collect { event ->
      when (event) {
        ...
        is NavigationEvent.LoadHtml -> loadDataWithBaseURL(
          event.baseUrl, event.html, event.mimeType, event.encoding, event.historyUrl
        )
    ...
    coroutineScope.launch {
      navigationEvents.emit(
        NavigationEvent.LoadHtml(
          html, baseUrl, mimeType, encoding, historyUrl
        )
      )
    }
  }
