...
class FileNMM : NativeMicroModule("file.std.dweb", "File Manager") {
    ...
    companion object {
         ...
         * 用于picker映射真实路径
         */
        private val pickerPathToRealPathMap = mutableMapOf<String, VirtualFsPath>()
    }
    ...
    fun IHandlerContext.getVfsPath(
        pathKey: String = "path",
    ) = if (pathKey.startsWith("/picker")) pickerPathToRealPathMap[pathKey]!! else getVirtualFsPath(
        ipc.remote,
    ...
    override suspend fun _bootstrap(bootstrapContext: BootstrapContext) {
                ...
                if (firstSegment == "sys") {
                    ...
                    return@respondLocalFile returnFile(
                        SystemFileSystem.source(
                            pickerPathToRealPathMap[request.url.encodedPath]!!.fsFullPath
        ...
        routes(
            ...
            "/virtualPathRealPathMapping" bind HttpMethod.Get to defineEmptyResponse {
                val pickerPath = request.queryAs<String>("pickerPath")
                val realPath = request.queryAs<VirtualFsPath>("realPath")
                
                pickerPathToRealPathMap[pickerPath] = realPath
            }
        ).protected("zip.browser.dweb")
        routes(
            ...
            "/picker" bind HttpMethod.Get to defineStringResponse {
                ...
                pickerPathToRealPathMap[pickerPathString] = vfsPath
