...
suspend fun NativeMicroModule.NativeRuntime.queryPermissions(vararg permissions: PERMISSION_ID) = nativeFetch(
...
suspend fun NativeMicroModule.NativeRuntime.queryPermission(permission: PERMISSION_ID) =
  queryPermissions(permission)[permission] ?: mapOf()

suspend fun NativeMicroModule.NativeRuntime.checkPermissions(vararg permissions: PERMISSION_ID) = nativeFetch(
  "file://permission.std.dweb/check?permissions=${
    permissions.joinToString(",").encodeURIComponent()
  }"
).json<Map<PERMISSION_ID, AuthorizationStatus>>()

suspend fun NativeMicroModule.NativeRuntime.checkPermission(permission: PERMISSION_ID) =
  checkPermissions(permission)[permission] ?: AuthorizationStatus.UNKNOWN

suspend fun NativeMicroModule.NativeRuntime.requestPermissions(vararg permissions: PERMISSION_ID) = nativeFetch(
  "file://permission.std.dweb/request?permissions=${
    permissions.joinToString(",").encodeURIComponent()
  }"
).json<Map<PERMISSION_ID, AuthorizationStatus>>()

suspend fun NativeMicroModule.NativeRuntime.requestPermission(permission: PERMISSION_ID) =
  requestPermissions(permission)[permission] ?: AuthorizationStatus.UNKNOWN


suspend fun NativeMicroModule.NativeRuntime.deletePermissions(
...
suspend fun NativeMicroModule.NativeRuntime.deletePermission(applicantMmid: MMID, permission: PERMISSION_ID) =
