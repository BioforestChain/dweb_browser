...
public class DwebVCData {
    public var vc: UIViewController
    public var prop: HelperPlatformDwebUIViewControllerProperty

    public init(vc: UIViewController, prop: HelperPlatformDwebUIViewControllerProperty) {
        self.vc = vc
        self.prop = prop
    }
}

public class DwebDeskVCStore: ObservableObject {
    public static let shared = DwebDeskVCStore()

    @Published public var vcs = [DwebVCData]()
    @Published public var navgationBarVisible: Visibility = .visible

    init() {
        regiserDeskEvent()
    }

    public func startUpNMMs(_ app: UIApplication) {
        #if DEBUG
            let debugMode = true
        #else
            let debugMode = false
