...
public val commonAsyncExceptionHandler: CoroutineExceptionHandler = CoroutineExceptionHandler { ctx, e ->
  printError(ctx.toString(), e.message, e)
  debugger(ctx, e)
}
public val globalEmptyScope: CoroutineScope = CoroutineScope(EmptyCoroutineContext + commonAsyncExceptionHandler)

public val defaultAsyncExceptionHandler: CoroutineContext = Dispatchers.Default + commonAsyncExceptionHandler
public val globalDefaultScope: CoroutineScope = CoroutineScope(defaultAsyncExceptionHandler)

public val defaultUnconfinedExceptionHandler: CoroutineContext = Dispatchers.Unconfined + commonAsyncExceptionHandler
...
public val mainAsyncExceptionHandler: CoroutineContext = SupervisorJob() + Dispatchers.Main + commonAsyncExceptionHandler
...
public val globalIoScope: CoroutineScope get() = gioScope ?: CoroutineScope(ioAsyncExceptionHandler).also { gioScope = it }
