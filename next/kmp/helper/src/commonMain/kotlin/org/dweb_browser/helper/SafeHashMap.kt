...
public class SafeHashMap<K, V>(public val origin: MutableMap<K, V> = mutableMapOf()) : MutableMap<K, V> {
  public val lock: SynchronizedObject = SynchronizedObject()
  public inline fun <T> sync(block: MutableMap<K, V>.() -> T): T = synchronized(lock) { origin.block() }
