...
const openNativeWindow_js_1 = require("../../helper/openNativeWindow.js");
const _createHttpDwebServer_js_1 = require("../http-server/$createHttpDwebServer.js");
const ipc_native2js_js_1 = require("./ipc.native2js.js");
let pre = 0;
const resolveTo = (0, createResolveTo_js_1.createResolveTo)(__dirname);
class ImportLinker {
    constructor(origin, 
    ...
    importMaps = []) {
        ...
        Object.defineProperty(this, "importMaps", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: importMaps
    ...
    link(url) {
        ...
        for (const item of this.importMaps) {
            if ((0, _ReqMatcher_js_1.$isMatchReq)(item.pathMatcher, parsed_url.pathname)) {
...
/** 响应错误信息 */
const _ipcErrorResponse = (requestMessage, ipc, statusCode, errorMessage) => {
    const headers = new IpcHeaders_js_1.IpcHeaders(CORS_HEADERS);
    ipc.postMessage(index_js_1.IpcResponse.fromText(requestMessage.req_id, statusCode, headers, errorMessage, ipc));
};
/** 响应实体内容 */
const _ipcSuccessResponse = (requestMessage, ipc, code) => {
    const headers = new IpcHeaders_js_1.IpcHeaders(CORS_HEADERS);
    headers.set("Content-Type", code.mime);
    ipc.postMessage(typeof code.data === "string"
        ? index_js_1.IpcResponse.fromText(requestMessage.req_id, 200, headers, code.data, ipc)
        : index_js_1.IpcResponse.fromBinary(requestMessage.req_id, 200, headers, code.data, ipc));
};
/** 基于 importLinker 响应内容 */
const _ipcResponseFromImportLinker = async (ipc, importLinker, request) => {
 ...
 */
class JsProcessNMM extends micro_module_native_js_1.NativeMicroModule {
    constructor() {
        super(...arguments);
        ...
        Object.defineProperty(this, "JS_PROCESS_WORKER_CODE", {
            enumerable: true,
            configurable: true,
            writable: true,
        ...
        Object.defineProperty(this, "INTERNAL_PATH", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: encodeURI("/<internal>")
        });
        // static singleton = once(() => new JsProcessManager());
    }
    async _bootstrap() {
        console.log("[js-process _bootstrap]");
        const mainServer = await (0, _createHttpDwebServer_js_1.createHttpDwebServer)(this, {});
        (await mainServer.listen()).onRequest(async (request, ipc) => {
            ...
            if (pathname.endsWith("/bootstrap.js")) {
                return ipc.postMessage(await index_js_1.IpcResponse.fromText(request.req_id, 200, new IpcHeaders_js_1.IpcHeaders({
                    "Content-Type": "text/javascript",
                }), await this.JS_PROCESS_WORKER_CODE(), ipc));
            }
            ipc.postMessage(await index_js_1.IpcResponse.fromResponse(request.req_id, await this.nativeFetch("file:///bundle/js-process" + request.parsed_url.pathname), ipc));
        });
        ...
        const apis = await (async () => {
            ...
            const nww = await (0, openNativeWindow_js_1.openNativeWindow)(
        ...
        const ipcProcessIdMap = new WeakMap();
        /// 创建 web worker
        this.registerCommonIpcOnMessageHandler({
            ...
            handler: async (args, ipc, requestMessage) => {
                ...
                const processIdMap = mapHelper_js_1.mapHelper.getOrPut(ipcProcessIdMap, ipc, () => new Map());
    ...
    async _shutdown() { }
    async createProcessAndRun(ipc, apis, bootstrap_url, entry = "/index.js", requestMessage) {
        /**
         * 用自己的域名的权限为它创建一个子域名
         ...
         */
        const httpDwebServer = await (0, _createHttpDwebServer_js_1.createHttpDwebServer)(this, {
            subdomain: ipc.remote.mmid,
        });
        /**
         * 远端是代码服务，所以这里是 client 的身份
         */
        const streamIpc = new ReadableStreamIpc_js_1.ReadableStreamIpc(ipc.remote, index_js_1.IPC_ROLE.CLIENT);
        ...
        const processInfo = await apis.createProcess(bootstrap_url, metadata, env, 
        // JSON.stringify(metadata),
        // JSON.stringify(env),
        (0, comlink_1.transfer)(channel_for_worker.port2, [channel_for_worker.port2]));
        /**
         * 将 js-worker 中的请求进行中转代理
         * 在Android和IOS中的Webview默认只能传输字符串
         */
        const ipc_to_worker = new MessagePortIpc_js_1.MessagePortIpc(channel_for_worker.port1, ipc.remote, index_js_1.IPC_ROLE.CLIENT);
        ipc_to_worker.onMessage((ipcMessage) => {
            if (ipcMessage.type === index_js_1.IPC_MESSAGE_TYPE.REQUEST &&
    ...
    async createIpc(ipc, apis, process_id, mmid) {
        const env = JSON.stringify({
            "ipc-support-protocols": "raw message_pack",
        });
        /**
        ...
        await apis.createIpc(process_id, mmid, (0, comlink_1.transfer)(channel_for_worker.port2, [channel_for_worker.port2]), env);
        // 把一个messageChange保存到全局对象
        return (0, ipc_native2js_js_1.saveNative2JsIpcPort)(channel_for_worker.port1);
    }
}
