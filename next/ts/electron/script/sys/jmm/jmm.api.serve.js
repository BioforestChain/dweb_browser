...
async function createApiServer() {
    // 为 下载页面做 准备
    this.apiServer = await (0, _createHttpDwebServer_js_1.createHttpDwebServer)(this, {
        subdomain: "api",
        port: 6363,
    });
...
async function getData(request, ipc) {
    const searchParams = request.parsed_url.searchParams;
    const url = searchParams.get("url");
    if (url === null)
        throw new Error(`${this.mmid} url === null`);
    const res = await fetch(url);
    ipc.postMessage(await index_js_1.IpcResponse.fromResponse(request.req_id, res, ipc, true));
}
async function appDownload(ipcRequest, ipc) {
    const search = ipcRequest.parsed_url.searchParams;
    const downloadUrl = search.get("url");
    const id = search.get("id");
    if (downloadUrl === null)
        throw new Error(`downloadUrl === null`);
    if (id === null)
    ...
    ipc.postMessage(await index_js_1.IpcResponse.fromStream(ipcRequest.req_id, 200, new index_js_1.IpcHeaders(), this.downloadStream, ipc));
    const tempPath = path_1.default.resolve(process.cwd(), `./temp/${id}.tar.gz`);
    const writeAblestream = fs_1.default.createWriteStream(tempPath, { flags: "w" });
    writeAblestream.on("close", () => _extract.bind(this)(id, tempPath, ipcRequest, ipc));
    (0, request_progress_1.default)((0, request_1.default)(downloadUrl), {})
...
async function onProgress(ipcRequest, ipc, state) {
    // // 测试关闭下载
...
async function _extract(id, tempPath, request, ipc) {
    const target = path_1.default.resolve(process.cwd(), `./apps`);
    // 判断 target 目录是否存在 不存在就创建目录
    if (!fs_1.default.existsSync(target)) {
        await promises_1.default.mkdir(target, {
            recersive: true,
        });
    }
    tar_1.default.x({
