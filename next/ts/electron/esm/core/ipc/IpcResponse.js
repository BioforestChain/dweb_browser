...
import { binaryToU8a } from "../../helper/binaryHelper.js";
import { IpcMessage, IPC_MESSAGE_TYPE } from "./const.js";
import { IpcBodySender, setStreamId } from "./IpcBodySender.js";
...
export class IpcResponse extends IpcMessage {
    constructor(req_id, statusCode, headers, body, ipc) {
        ...
        Object.defineProperty(this, "ipcResMessage", {
            ...
            configurable: true,
            writable: true,
            value: once(() => new IpcResMessage(this.req_id, this.statusCode, this.headers.toJSON(), this.body.metaBody))
        });
    ...
    get ipcHeaders() {
        return (__classPrivateFieldSet(this, _IpcResponse_ipcHeaders, __classPrivateFieldGet(this, _IpcResponse_ipcHeaders, "f") ?? new IpcHeaders(this.headers), "f"));
    }
    toResponse(url) {
    ...
    asBinary = false // response.headers.get("content-length") !== null
        ...
        if (asBinary || response.body == undefined) {
            ipcBody = IpcBodySender.from(binaryToU8a(await response.arrayBuffer()), ipc);
        }
        else {
            setStreamId(response.body, response.url);
            ipcBody = IpcBodySender.from(response.body, ipc);
        }
        return new IpcResponse(req_id, response.status, new IpcHeaders(response.headers), ipcBody, ipc);
    }
    ...
    static fromText(req_id, statusCode, headers = new IpcHeaders(), text, ipc) {
        ...
        return new IpcResponse(req_id, statusCode, headers, IpcBodySender.from(text, ipc), ipc);
    }
    static fromBinary(req_id, statusCode, headers = new IpcHeaders(), binary, ipc) {
        ...
        return new IpcResponse(req_id, statusCode, headers, IpcBodySender.from(binaryToU8a(binary), ipc), ipc);
    }
    static fromStream(req_id, statusCode, headers = new IpcHeaders(), stream, ipc) {
        headers.init("Content-Type", "application/octet-stream");
        const ipcResponse = new IpcResponse(req_id, statusCode, headers, IpcBodySender.from(stream, ipc), ipc);
        return ipcResponse;
    }
    toJSON() {
...
export class IpcResMessage extends IpcMessage {
