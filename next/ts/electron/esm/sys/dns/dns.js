...
import { connectMicroModules, } from "../../core/nativeConnect.js";
import { $readRequestAsIpcRequest } from "../../helper/$readRequestAsIpcRequest.js";
...
export class DnsNMM extends NativeMicroModule {
    ...
    async [connectTo_symbol](fromMM, toMmid, reason) {
        // v2.0
        // 创建连接
        const fromMMconnectsMap = mapHelper.getOrPut(this.mmConnectsMap, fromMM, () => new Map());
        const po = mapHelper.getOrPut(fromMMconnectsMap, toMmid, () => {
            ...
            (async () => {
                ...
                if (ipcForToMM) {
                    ...
                    const toMMconnectsMap = mapHelper.getOrPut(this.mmConnectsMap, toMM, () => new Map());
                    mapHelper.getOrPut(toMMconnectsMap, fromMM.mmid, () => {
                        const toMMPromise = new PromiseOut();
                        ipcForToMM.onClose(() => {
    ...
    _bootstrap() {
        ...
        this.registerCommonIpcOnMessageHandler({
            ...
            handler: async (args, client_ipc, request) => {
                ...
                return IpcResponse.fromJson(request.req_id, 200, new IpcHeaders({
        ...
        this.registerCommonIpcOnMessageHandler({
            ...
            output: "boolean",
            handler: async (args, client_ipc, request) => {
                const { JsMicroModule } = await import("../jmm/micro-module.js.js");
                const { JmmMetadata } = await import("../jmm/JmmMetadata.js");
        ...
        this._after_shutdown_signal.listen(nativeFetchAdaptersManager.append(async (fromMM, parsedUrl, requestInit) => {
            // 测试代码
