...
import { IPC_MESSAGE_TYPE, IPC_ROLE, IpcResponse, } from "../../core/ipc/index.js";
...
import { createResolveTo } from "../../helper/createResolveTo.js";
import { mapHelper } from "../../helper/mapHelper.js";
import { openNativeWindow } from "../../helper/openNativeWindow.js";
import { createHttpDwebServer } from "../http-server/$createHttpDwebServer.js";
import { saveNative2JsIpcPort } from "./ipc.native2js.js";
let pre = 0;
    ...
    async _bootstrap() {
        ...
        (await mainServer.listen()).onRequest(async (request, ipc) => {
            ...
            if (pathname.endsWith("/bootstrap.js")) {
                return ipc.postMessage(await IpcResponse.fromText(request.req_id, 200, new IpcHeaders({
                    "Content-Type": "text/javascript",
                }), await this.JS_PROCESS_WORKER_CODE(), ipc));
            }
            ipc.postMessage(await IpcResponse.fromResponse(request.req_id, await this.nativeFetch("file:///bundle/js-process" + request.parsed_url.pathname), ipc));
        });
        ...
        this.registerCommonIpcOnMessageHandler({
            ...
            handler: async (args, ipc, requestMessage) => {
                ...
                const processIdMap = mapHelper.getOrPut(ipcProcessIdMap, ipc, () => new Map());
