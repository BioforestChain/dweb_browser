...
import excuteJavascriptCode from "./multi-webview-excute-javascript.js";
...
import { exportApis, mainApis } from "../../../helper/openNativeWindow.preload.js";
import { getButtomBarState } from "./components/multi-webview-comp-safe-area.shim.js";
...
let ViewTree = class ViewTree extends LitElement {
    constructor() {
        ...
        Object.defineProperty(this, "safeAreaGetState", {
            ...
            value: () => {
                ...
                const bottomBarState = getButtomBarState(navigationBarState, this.isShowVirtualKeyboard, this.virtualKeyboardState);
                return {
        ...
        Object.defineProperty(this, "safeAreaNeedUpdate", {
            ...
            writable: true,
            value: () => {
                ipcRenderer.send("safe_area_update", new URL(this.webviews[this.webviews.length - 1].src).host.replace("www.", "api."), this.safeAreaGetState());
            }
        });
    ...
    async onWebviewReady(webview, ele) {
        ...
        webview.doReady(ele);
        mainApis.denyWindowOpenHandler(webview.webContentId, proxy((detail) => {
            this.openWebview(detail.url);
        }));
        mainApis.onDestroy(webview.webContentId, proxy(() => {
    ...
    async onDevtoolReady(webview, ele_devTool) {
        ...
        await mainApis.openDevTools(webview.webContentId, undefined, webview.webContentId_devTools);
    }
    ...
    render() {
      ...
      <div class="app-container">
        ...
        >
          ${repeat(this.webviews, (webview) => webview.src, (webview, index) => {
            if (index === 0) {
                return html `
          ...
          ${repeat(this.webviews, (dialog) => dialog.id, (webview) => {
          ...
          ${repeat(this.webviews, webview => webview.src, (webview, index) => {
            if (index === 0) {
                return html `
                    ${when(this.isShowVirtualKeyboard, () => html `
