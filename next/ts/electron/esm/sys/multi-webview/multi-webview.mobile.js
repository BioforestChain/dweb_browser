...
import { locks } from "../../helper/locksManager.js";
...
import { open, closeFocusedWindow, openDownloadPage, barGetState, barSetState, safeAreaGetState, safeAreaSetState, virtualKeyboardGetState, virtualKeyboardSetState, toastShow, shareShare, toggleTorch, torchState, haptics, biometricsMock } from "./multi-webview.mobile.handler.js";
/**
 * 构建一个视图树
...
export class MultiWebviewNMM extends NativeMicroModule {
    ...
    async _bootstrap(context) {
        ...
        (await httpDwebServer.listen()).onRequest(async (request, ipc) => {
            ipc.postMessage(await IpcResponse.fromResponse(request.req_id, await this.nativeFetch("file:///bundle/multi-webview" + request.parsed_url.pathname), ipc));
        });
