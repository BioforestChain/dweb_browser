...
extension BrowserContainerViewController: BrowserTabViewControllerDelegate {
    ...
    func updateToolbarButtons() {
        let webView = tabViewControllers[currentTabIndex].contentView.webView
        let homePage = tabViewControllers[currentTabIndex].contentView.homePageView
        
        let homePageIsHidden = homePage.alpha == 0
        contentView.toolbar.goBackButton.isEnabled = webView.canGoBack  && homePageIsHidden
        contentView.toolbar.goForwardButton.isEnabled = webView.canGoForward  && homePageIsHidden
        contentView.toolbar.bookmarkButton.isEnabled = webView.estimatedProgress > 0.7 && homePageIsHidden
        contentView.toolbar.homeButton.isEnabled = tabViewControllers[currentTabIndex].hasLoadedUrl  && homePageIsHidden
