 ...
 */
export class AddRoutesToHttp extends BaseAddRoutesToHttp<VirtualKeyboardNMM>{

  constructor(
    nmm: VirtualKeyboardNMM,
  ...
  //   if(id === undefined) throw new Error(`virtual-keyboard.nativeui.sys.dweb _httpIpcOnEventRequestDistributeOperationReturn id === undefined`)
  //   const _d = this._reqs.get(parseInt(id))
  //   if(_d === undefined) throw new Error(`virtual-keyboard.nativeui.sys.dweb _httpIpcOnEventRequestDistributeOperationReturn d === undefined`)
  //   const _url = url.parse(data.url.split("app_url=")[1])
  //   const app_url = `${_url.protocol}//${_url.host}`
    
  //   if(
  //     !this._isObserve /** 是否还在监听中 */
  //     || _d.pathname === '/virtual-keyboard.nativeui.sys.dweb/getState' /** 操作的路由不能够是 getState */ 
  //   ) return;
