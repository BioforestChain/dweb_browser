...
export class StatusbarNativeUiNMM extends NativeMicroModule {
  ...
  _bootstrap = async (context: $BootstrapContext) => {
    ...
    
    {
      ...
      this.httpNMM.addRoute(`/status-bar-ui/wait_for_operation`, this._waitForOperation)
      this.httpNMM.addRoute(`/status-bar-ui/operation_return`, this._operationReturn)
    }
  ...
  private _operationReturn = async (req: IncomingMessage, res: OutgoingMessage) => {
    ...
    if(typeof id !== "string") throw new Error(`${this.mmid}id !== string`);
