 ...
 */
export class AddRoutesToHttp extends BaseAddRoutesToHttp<SafeAreaNMM>{
  ...
  _httpIpcOnEventRequestDistribute = async (ipcEvent: IpcEvent, httpIpc: Ipc) => {
    ...
    switch(pathname){
      ...
      case "/safe-area.nativeui.sys.dweb/startObserve":
        ...
        break;
      case "/safe-area.nativeui.sys.dweb/stopObserve":
      ...
      case "/internal/observe":
        ...
        break;
      default: throw new Error(`${this._nmm.mmid} add-routes-to-https.cts _httpIpcOnEventRequestDistribute 还有没有处理的路由 ${pathname}`)
    }
  }
  ...
  _httpIpcOnEventRequestDistributeOperationReturn = async (data: $RequestDistributeIpcEventData, httpIpc: Ipc) => {
    log.green(`${this._nmm.mmid} add-routes-to-http.cts data`)
    ...
    if(observe === undefined) {
      log.red(`${this._nmm.mmid} add-routes-to-https.cts observe === undefined ${app_url}`)
      return;
    }
