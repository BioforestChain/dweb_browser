...
export class HttpConnect{
  ...
  constructor(
      private readonly _nmm: VirtualKeyboardNMM,
  ...
  _httpIpcOnMessage = async (ipcMessage: $IpcMessage, httpIpc: Ipc) => {
      switch(ipcMessage.type){
          case IPC_MESSAGE_TYPE.EVENT:
              ...
              break;
          default: throw new Error(`virtual-keyboard.nativeui.sys.dweb http-ipc-on-message 还有没有处理的 类型`)
      }
  }
  ...
  _httpIpcOnEvent = async (ipcMessage: IpcEvent, httpIpc: Ipc) => {
    switch(ipcMessage.name){
        case "request/distribute":
            ...
            break;
        default: throw new Error(`[virtual-keyboard.nativeui.sys.dweb htp-ipc-on-event] 还没没处理的 message ${ipcMessage.name}`);
    }
  } 
  ...
  _httpIpcOnEventRequestDistribute = async (ipcEvent: IpcEvent, httpIpc: Ipc) => {
    ...
    switch(pathname){
      case "/virtual-keyboard.nativeui.sys.dweb/getState":
        this._httpIpcOnEventRequestDistributeGetState(data, httpIpc);
        break;
      case "/virtual-keyboard.nativeui.sys.dweb/setState":
      ...
      case "/virtual-keyboard-ui/wait_for_operation":
        ...
        break;
      case "/virtual-keyboard-ui/operation_return":
      ...
      //   break;
      default: throw new Error(`virtual-keyboard.nativeui.sys.dweb http-connect _httpIpcOnEventRequestDistribute 还有没有处理的路由 ${pathname}`)
    }
  }
  ...
  _httpIpcOnEventRequestDistributeWaitForOperationReturn = async (data: $RequestDistributeIpcEventData, httpIpc: Ipc) => {
    ...
    if(id === undefined) throw new Error(`virtual-keyboard.nativeui.sys.dweb _httpIpcOnEventRequestDistributeWaitForOperationReturn id === undefined`)
    const _d = this._reqs.get(parseInt(id))
    if(_d === undefined) throw new Error(`virtual-keyboard.nativeui.sys.dweb _httpIpcOnEventRequestDistributeWaitForOperationReturn d === undefined`)
    httpIpc.postMessage(
      IpcEvent.fromText(
        "http.sys.dweb",
        JSON.stringify({
  ...
  _httpIpcOnEventRequestDistributeSetState = async (data: $RequestDistributeIpcEventData, httpIpc: Ipc) => {
    ...
    throw new Error(`virtual-keyboard.nativeui.sys.dweb _httpIpcOnEventRequestDistributeSetState 还有没处理的 setState 请求`)
  }
  ...
  _postMessageToUI = async (body:Object, httpIpc: Ipc, from: string) => {
    ...
    if(route === undefined){
      throw new Error(`virtual-keyboard.nativeui.sys.dweb htt-connect _postMessageToUI route === undefined`)
    }
     // 把请求发送给 UI
    httpIpc.postMessage(
      IpcEvent.fromText(
