...
class JsMicroModule extends micro_module_cjs_1.MicroModule {
    ...
    /** 每个 JMM 启动都要依赖于某一个js */
    async _bootstrap() {
        const process_id = (this._process_id = await this.fetch(`file://js.sys.dweb/create-process?main_code=${encodeURIComponent('console.log("worker running!", location.href)')}`).number());
    }
    async _connect() {
