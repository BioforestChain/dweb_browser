import { $once } from "../../helper/decorator/$once.ts";
import {
  createFetchHandler,
  type $OnFetch,
} from "../../helper/fun/ipcFetchHelper.js";
import { buildUrl } from "../../helper/fun/urlHelper.ts";
import type { $ReqMatcher } from "../../helper/type/$ReqMatcher.js";
...
export class HttpDwebServer {
  ...
  listen = $once(async (...onFetchs: $OnFetch[]) => {
    ...
    serverIpc.onRequest("http-listen").collect(async (event) => {
      ...
      const response = await event.consumeAwaitMapNotNull((request) =>
...
/** 开始监听端口和域名 */
const startHttpDwebServer = async (
  microModule: MicroModuleRuntime,
  options: $DwebHttpServerOptions
) => {
...
const listenHttpDwebServer = async (
  ...
  const serverIpc =
    ...
    (await (async () => {
      ...
      return await httpIpc.fork(
        undefined,
        undefined,
        true,
        "listenHttpDwebServer"
      );
    })());
...
/** 停止监听端口和域名 */
const closeHttpDwebServer = async (
  microModule: MicroModuleRuntime,
  options: $DwebHttpServerOptions
) => {
  return await microModule
