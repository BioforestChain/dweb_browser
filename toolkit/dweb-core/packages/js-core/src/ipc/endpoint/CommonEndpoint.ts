...
import { Channel } from "../../../helper/Channel.js";
import {
...
import {
  ...
  type $EndpointRawMessage,
} from "./EndpointMessage.ts";
      ...
      for await (const endpointMessage of this.endpointMsgChannel) {
        switch (endpointMessage.type) {
          case ENDPOINT_MESSAGE_TYPE.IPC: {
            ...
            producer.producer.trySend(
              $normalizeIpcMessage(
                endpointMessage.ipcMessage,
                await producer.ipcPo.promise
              )
            );
            break;
          }
