...
@customElement("dweb-bluetooth-device-list")
export class DwebBluetoothDeviceList extends LitElement{
  static override styles = createAllCSS();
  @property() list: $Device[] = []
  @property() connectingDeviceId: string = ""
  @property() connectedDeviceId: string = "";
  constructor(){
    super();
  }
  render(){
    console.log('list: ', this.list)
    console.log('this.connectingDeviceId: ',this.connectingDeviceId)
          ...
          (device, index) => html`
            <li class="list_item">
              <span 
                class=${classMap({
                  name: true,
                  name_active: device.deviceId == this.connectedDeviceId
                })}>
                  ${device.deviceName}
                </span>
              <span 
                class=${classMap({
                  status: true,
                  status_active: device.deviceId !== this.connectedDeviceId && device.deviceId !== this.connectingDeviceId
                })}
              >
                未连接
              </span>
              <span 
                class=${classMap({
                  ...
                  status_connected_active: device.deviceId === this.connectedDeviceId
                })}
              >
                已连接
              </span>
              <span 
                class=${classMap({
                  loading: true,
                  loading_active: device.deviceId === this.connectingDeviceId
                })}
              >
                <svg 
                  t="1687161242311" 
