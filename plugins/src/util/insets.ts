...
export const insetsToDom = (insets: $Insets) =>
  new DOMInsets(insets.left, insets.top, insets.right, insets.bottom);
...
export class DOMInsets {
  ...
  effect(options: $InsetsToCssOptions) {
    const eleId =
      options.css_var_prefix?.concat("<<prefix") ??
      options.css_var_name?.concat("<<name") ??
      "default";
...
const formatInsetsToCssOptions = (options: $InsetsToCssOptions = {}) => {
  const css_var_prefix =
    options.css_var_prefix ??
    (options.css_var_name ? `--${options.css_var_name}-inset-` : `--inset-`);
